...
import { generateApplicationId, maskTckn } from '../../lib/applications-store';
...
// In a real system this comes from YOKSIS for the logged-in student.
// Here we simulate by combining the authenticated user with a stub current-program record.
const SIMULATED_CURRENT_PROGRAM: Record<string, { university: string; program: string; studentNumber: string }> = {
  '1': {
...
export function ApplicationForm({ user, onSave, onCancel }: ApplicationFormProps) {
  ...
  // Simulate YOKSIS / OSYM data fetch keyed off the authenticated user.
  useEffect(() => {
    let cancelled = false;
    const fetchExternalData = async () => {
      ...
      const mockApiDown = false; // toggle to demo the fallback branch
      if (mockApiDown) {
        setApiDown(true);
        setIsLoading(false);
        return;
      ...
      const profile = SIMULATED_CURRENT_PROGRAM[user.id] || {
      ...
      setFormData(prev => ({
        ...
        currentUniversity: profile.university,
  ...
  }, [user.id, user.name, user.surname, user.tckn]);

  const validateForm = () => {
    const newErrors: Record<string, string> = {};

    if (!formData.targetProgram) newErrors.targetProgram = 'Lütfen bir program seçiniz';
    ...
    if (!formData.osymScore) {
      ...
      if (isNaN(score) || score < 0 || score > 600) newErrors.osymScore = 'Geçersiz ÖSYM puanı';
    }

    if (!formData.osymYear) newErrors.osymYear = 'ÖSYM sınav yılı gereklidir';
  ...
  const handleSaveDraft = () => {
    ...
    onSave({ ...formData, applicationId, status: 'draft' });
  ...
  const handleContinue = () => {
    if (validateForm()) {
      const applicationId = generateApplicationId();
      onSave({ ...formData, applicationId, status: 'in_progress' });
        ...
        <form className="space-y-6">
          <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="tckn" value={maskTckn(formData.tckn)} readOnly className="bg-gray-50" />
                  {!isLoading && <CheckCircle2 className="absolute right-3 top-2.5 h-4 w-4 text-green-600" />}
                </div>
              </div>
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formData.targetProgram} onValueChange={(value) => setFormData({ ...formData, targetProgram: value })}>
                  ...
                  <SelectContent>
                    <SelectItem value="Computer Engineering">Bilgisayar Mühendisliği</SelectItem>
                    <SelectItem value="Electrical Engineering">Elektrik-Elektronik Mühendisliği</SelectItem>
                    <SelectItem value="Mechanical Engineering">Makine Mühendisliği</SelectItem>
                    <SelectItem value="Industrial Engineering">Endüstri Mühendisliği</SelectItem>
                    <SelectItem value="Civil Engineering">İnşaat Mühendisliği</SelectItem>
                    <SelectItem value="Architecture">Mimarlık</SelectItem>
                  </SelectContent>
                </Select>
                {errors.targetProgram && <p className="text-xs text-red-600">{errors.targetProgram}</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="targetSemester">Hedef Dönem *</Label>
                <Select value={formData.targetSemester} onValueChange={(value) => setFormData({ ...formData, targetSemester: value })}>
                ...
                {errors.targetSemester && <p className="text-xs text-red-600">{errors.targetSemester}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                {errors.osymScore && <p className="text-xs text-red-600">{errors.osymScore}</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="osymYear">ÖSYM Sınav Yılı *</Label>
                <Select value={formData.osymYear} onValueChange={(value) => setFormData({ ...formData, osymYear: value })}>
                ...
                {errors.osymYear && <p className="text-xs text-red-600">{errors.osymYear}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between pt-4">
            ...
            <div className="space-x-3">
              <Button type="button" variant="outline" onClick={handleSaveDraft} disabled={isLoading || !isFormValid()}>
