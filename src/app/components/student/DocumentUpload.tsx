...
import { useApplications, maskTckn } from '../../lib/applications-store';
...
const BASE_DOCUMENTS: DocumentType[] = [
  { id: 'transcript', name: 'Resmi Transkript', description: 'Mevcut üniversitenizden alınan güncel transkript', required: true, maxSize: 10, acceptedFormats: ['PDF', 'JPG', 'PNG'] },
  { id: 'osym_result', name: 'ÖSYM Sonuç Belgesi', description: 'Resmi ÖSYM puan belgesi', required: true, maxSize: 10, acceptedFormats: ['PDF', 'JPG', 'PNG'] },
  { id: 'curriculum', name: 'Ders Planı (Müfredat)', description: 'Mevcut üniversitenizdeki program müfredatı', required: true, maxSize: 10, acceptedFormats: ['PDF'] },
  { id: 'student_certificate', name: 'Öğrenci Belgesi', description: 'Aktif öğrencilik durumunu gösteren belge', required: true, maxSize: 10, acceptedFormats: ['PDF', 'JPG', 'PNG'] },
  { id: 'language_proficiency', name: 'Dil Yeterlilik Belgesi', description: 'Varsa TOEFL, IELTS veya YDS belgesi', required: false, maxSize: 10, acceptedFormats: ['PDF', 'JPG', 'PNG'] },
  { id: 'course_contents', name: 'Ders İçerikleri', description: 'Tamamlanan derslerin detaylı içerikleri/syllabusları', required: true, maxSize: 10, acceptedFormats: ['PDF'] }
];

export function DocumentUpload({ user, applicationId, applicationData, onComplete, onBack }: DocumentUploadProps) {
  ...
  const generateFileName = (docTypeId: string, originalName: string) => {
    ...
    const masked = maskTckn(applicationData?.tckn || user.tckn);
    return `${applicationId}_${docTypeId}_${masked}.${ext}`;
  };
  ...
  const handleFileUpload = (docType: DocumentType, file: File) => {
    ...
    const newEntry = {
      ...
      history: existing
        ? [{ name: existing.file.name, date: new Date().toLocaleString() }, ...existing.history]
        : []
    };

    setUploadedDocs(prev => ({ ...prev, [docType.id]: newEntry }));
  ...
  const canSubmit = () => {
    ...
    return requiredDocs.every(doc => uploadedDocs[doc.id] && uploadedDocs[doc.id].status === 'valid');
  };
  ...
  const handleSubmit = () => {
    ...
    // Build a typed Application from the form draft and the uploaded documents,
    // then push it into the persistent store. This is the moment a "real" application
    // exists in the system from the student's perspective.
    const now = new Date();
    const documents: ApplicationDocument[] = Object.entries(uploadedDocs)
      .filter(([, v]) => v.status === 'valid')
      .map(([key, v], i) => ({
        id: `${applicationId}-d${i + 1}`,
        type: (key as ApplicationDocument['type']),
        fileName: generateFileName(key, v.file.name),
        uploadedAt: now,
        verified: false
      }));

    const targetSemester = applicationData?.targetSemester === '5' ? 5 : 3;
    ...
    const newApp: Application = {
      ...
      tckn: user.tckn,
      currentProgram: applicationData?.currentProgram
        ? `${applicationData.currentProgram} - ${applicationData.currentUniversity || ''}`.trim()
        : 'Unknown',
      targetProgram: applicationData?.targetProgram || 'Unknown',
      targetSemester,
      gpa: parseFloat(applicationData?.gpa) || 0,
      osymScore: parseFloat(applicationData?.osymScore) || 0,
      status: 'submitted',
      submittedAt: now,
      documents,
      timeline: [
        { status: 'submitted', timestamp: now, actor: `${user.name} ${user.surname}` },
        { status: 'intake_verification', timestamp: now, actor: 'System' }
      ]
    };

    // Small delay to show the spinner-like UX from the original implementation.
    setTimeout(() => {
      addApplication(newApp);
      toast.success('Başvurunuz başarıyla gönderildi', {
        description: `${applicationId} ÖİDB doğrulama kuyruğuna eklendi.`
      });
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    {docType.required && <span className="text-xs text-red-600">*Zorunlu</span>}
                    {isUploaded && isValid && <CheckCircle2 className="w-5 h-5 text-green-600" />}
                    {isUploaded && !isValid && <AlertCircle className="w-5 h-5 text-red-600" />}
                  </div>
                    ...
                    <div className="mt-4 space-y-3">
                      <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="flex items-center space-x-2">
                                ...
                                {uploaded.version > 1 && <Badge variant="outline" className="text-[10px] h-4">v{uploaded.version}</Badge>}
                              </div>
                          ...
                          <div className="flex items-center space-x-2">
                            <Button size="sm" variant="ghost" onClick={() => handleRemoveFile(docType.id)}>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            {canSubmit() ? <CheckCircle2 className="w-8 h-8 text-green-600" /> : <AlertCircle className="w-8 h-8 text-yellow-600" />}
          </div>
        </div>
      </Card>
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button onClick={handleSubmit} disabled={!canSubmit() || isSubmitting} style={{ backgroundColor: '#C00000' }}>
          <Send className="w-4 h-4 mr-2" />
          {isSubmitting ? 'Gönderiliyor...' : 'Başvuruyu Tamamla ve Gönder'}
        </Button>
      </div>
